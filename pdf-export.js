...
async function generatePDF(puzzlesData, trimSizeStr, solutionsPerPage, pageMargin) {
    ...
    function drawPuzzlePage(puzzleData, puzzleNum, isSolution) {
        ...
        const titleH         = titleText !== null ? 0.5 : 0;    // height reserved for title
        const panelPad       = MARGIN;                           // inner padding of the panel
        const clueAreaHeight = (() => {
            // Estimate rows of clues needed
            const nWords   = result.placedWords.length;
            const nRows    = Math.ceil(nWords / s.clueCols);
            const lh       = 11 / 72 + 0.12;
            return 0.35 + nRows * lh + 0.2;                     // gap + rows + bottom pad
        })();
        const maxGridH = usableH - titleH - clueAreaHeight;
        ...
        // Centre the whole block vertically on the page
        const blockTopY = (H - contentH) / 2;

        const titleY = blockTopY + (titleText !== null ? 0.3 : 0);
        const gridY  = blockTopY + titleH;
        const gridX  = (W - gridW) / 2;

        // ── 2. White rounded panel — sized exactly around content ─────────────
        if (!isSolution && s.bgImageData) {
            const panelInset  = 0.18;   // background breathing room on sides
            const panelX      = panelInset;
            const panelY      = blockTopY - panelPad * 0.5;
            const panelW      = W - 2 * panelInset;
            const panelH      = contentH + panelPad;
            const panelRadius = 0.18;

            try {
                pdf.setFillColor(255, 255, 255);
                pdf.setDrawColor(255, 255, 255);
                pdf.setGState(pdf.GState({ opacity: 0.94 }));
